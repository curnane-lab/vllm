...
class FullAttentionManager(SingleTypeKVCacheManager):
    ...
    def _cache_partial_tail_block(
        ...
        block are intentionally skipped. An explicit Mamba checkpoint boundary
        is registered the same way so sibling requests sharing the prefix can
        hit at the checkpoint.
        """
        hash_block_size = self.block_pool.hash_block_size
...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def __init__(
        ...
        if self.mamba_cache_mode == "align":
            # Request ID -> checkpoint hash registered but not yet committed by
            # a forward (application-directed checkpoints).
            self._checkpoint_hashes: dict[str, BlockHashWithGroupId] = {}
            # Same-step paired consumers: exempt from the same-step block guard
            # (the two-phase layer execution orders the producer's state write
            # before the consumer's read within one forward).
            self._same_step_paired_reqs: set[str] = set()
    ...
    def get_num_blocks_to_allocate(
        ...
        if (
            ...
            and request_id not in self._same_step_paired_reqs
        ):
    ...
    def pop_blocks_for_free(self, request_id: str) -> list[KVCacheBlock]:
        if self.mamba_cache_mode == "align":
            ...
            if checkpoint_hash is not None:
                # Request freed before/after its checkpoint forward: release
                # the readiness gate so waiting siblings are not stuck on a
                # producer that will never commit (or has already committed).
                self.block_pool.mark_block_hash_ready(checkpoint_hash)
            self._same_step_paired_reqs.discard(request_id)
    ...
    def _cache_checkpoint_block(
        ...
        """Register the mamba state snapshot at ``checkpoint_position``.

        The snapshot lives in the block column holding the state after
        ``checkpoint_position`` tokens; its cache entry is marked unready until
        the forward that produces the state completes.
        """
        hash_block_size = self.block_pool.hash_block_size
        ...
        if checkpoint_position % self.block_size == 0:
            # Block-aligned checkpoint: register the block as a full block so
            # its hash matches the other groups' full-block hashes (required
            # for cross-group lookup), and so consumers resume exactly at a
            # block boundary. `cache_full_blocks` resolves the block-level
            # hash the same way for every group.
            self.block_pool.cache_full_blocks(
                request=request,
                blocks=blocks,
            ...
            resolved_hashes = resolve_block_hashes(
                request.block_hashes, hash_block_size, self.block_size
            )
            checkpoint_hash = make_block_hash_with_group_id(
                resolved_hashes[block_idx], self.kv_cache_group_id
            )
        else:
            # Sub-block checkpoint: register a partial-hash entry reachable at
            # the fine-grained boundary; resuming requests copy-on-write the
            # snapshot block.
        ...
        if checkpoint_position % self.block_size != 0:
            # The producer's continuing chunk overwrites this column, so the
            # request must copy-on-write the snapshot block before it runs.
    ...
    def prepare_same_step_mamba_consumer(
        self,
        request_id: str,
        source_blocks: Sequence[KVCacheBlock],
        checkpoint_position: int,
    ) -> None:
        """Register a same-step paired consumer of a checkpoint registered
        this pass. The consumer is exempt from the same-step block guard: the
        two-phase layer execution writes the producer's shared state before
        the consumer's phase reads it."""
        ckpt_block = source_blocks[-1]
        self._same_step_source_blocks[request_id] = ckpt_block
        self._same_step_paired_reqs.add(request_id)
        if checkpoint_position % self.block_size != 0:
            # Sub-block checkpoint: the consumer's suffix chunk would
            # overwrite the shared snapshot column, so it must copy-on-write.
            self._partial_hit_reqs[request_id] = (
                checkpoint_position // self.block_size,
                ckpt_block,
            )
    ...
    def cache_blocks(
        ...
        if checkpoint_position is not None:
            ...
            # With an explicit checkpoint, only the declared boundary is
            # cached; the state becomes visible once its forward completes.
